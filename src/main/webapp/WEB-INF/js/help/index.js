...
function showSideMenu(menuList, parent) {
    for(var menu in menuList) {
        ...
        if(typeof(menuList[menu].children) != 'undefined') {
            ...
            $(li).append("<a href='javascript:void(0)'><i class='icon-layers'></i> <span class='menu-text'>" + menuList[menu].text + "</span></a>").appendTo(parent);
        ...
        //如果该节点没有子节点，则直接将该节点li以及文本创建好直接添加到父亲节点中
            $("<li id='"+iid+"' class='menu-list' onclick=getactionLi('"+iid+"')></li>").append("<a href='javascript:void(0)' onclick=getUrl('" + menuList[menu].text+"','"+menuList[menu].id+"','"+menuList[menu].url+"')> <span class='menu-text'>" + menuList[menu].text + "</span></a>").appendTo(parent);
        }
    }
}
