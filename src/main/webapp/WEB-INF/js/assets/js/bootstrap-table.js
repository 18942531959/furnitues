  ...
  class BootstrapTable {
    ...
    initBody (fixedScroll) {
      ...
      this.$body.find('> tr[data-index] > td').off('click dblclick').on('click dblclick', ({currentTarget, type, target}) => {
        ...
        const $tr = $td.parent()
        const item = this.data[$tr.data('index.jsp')]
      ...
      this.$selectItem.off('click').on('click', e => {
        ...
        const $this = $(e.currentTarget)
        this.check_($this.prop('checked'), $this.data('index.jsp'))
      })
    ...
    updateRows () {
      this.$selectItem.each((i, el) => {
        this.data[$(el).data('index.jsp')][this.header.stateField] = $(el).prop('checked')
      })
    }
    ...
    insertRow (params) {
      if (!params.hasOwnProperty('index.jsp') || !params.hasOwnProperty('row')) {
        return
      }
    ...
    toggleRow (params, visible) {
      ...
      if (params.hasOwnProperty('index.jsp')) {
    ...
    expandAllRows (isSubTable) {
      if (isSubTable) {
        ...
        for (let i = 0; i < trs.length; i++) {
          this.expandRow_(true, $(trs[i]).data('index.jsp'))
        }
      }
    }
    ...
    collapseAllRows (isSubTable) {
      if (isSubTable) {
        ...
        const trs = this.$body.children()
        for (let i = 0; i < trs.length; i++) {
          this.expandRow_(false, $(trs[i]).data('index.jsp'))
        }
      }
    }
